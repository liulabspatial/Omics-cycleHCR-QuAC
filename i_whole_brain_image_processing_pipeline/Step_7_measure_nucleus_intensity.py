"""Step 7: single-nucleus protein intensity quantification.

Runs directly in a local python environment (see requirements.txt)

Reads the cropped single-nucleus images from Step 6, averages the non-zero
voxels of each nucleus in each channel, and writes one .h5ad file per subfolder.
"""
...
base_dir = "./Step6_crop"      # <- change to your path

# subfolders of base_dir to process
folder_ids = range(0, 26)      # <- change to your subfolders
...
def main():
    ...
    for folder_id in folder_ids:
        ...
        if not os.path.isdir(input_dir):
            print(f"Missing directory, skipping: {input_dir}")
            continue
        ...
        if not data:
            print(f"No matching images found in {input_dir}, skipping.")
            continue
