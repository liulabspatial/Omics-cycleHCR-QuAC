...
expr <- read.csv("input/R_26clusters_expression_zscore.csv", row.names=1, check.names=FALSE)

# Load cluster marker labels
cluster_markers <- read.csv("input/R_26clusters_marker_labels.csv", row.names=1, check.names=FALSE)
...
pheatmap(
  ...
  clustering_distance_cols = "correlation",  # canberra, manhattan, correlation, euclidean
